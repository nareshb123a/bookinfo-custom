...
detailsRatingsHostname = "details-ratings" if (os.environ.get("DETAILS_RATINGS_HOSTNAME") is None) else os.environ.get("DETAILS_RATINGS_HOSTNAME")
...
details_ratings = {
    "name": "http://{0}{1}:{2}".format(detailsRatingsHostname, servicesDomain, ratingsPort),
    "endpoint": "ratings",
    "children": []
}
...
def front():
    ...
    print(f'user after headers1: {user}')
    product = getProduct(product_id)
    print(f'product after headers1: {product}')
    detailsStatus, details = getProductDetails(product_id, headers)
    print(f'detailstatus: {detailsStatus}')
    ...
    print(f'reviewsStatus: {reviewsStatus}')
...
def getProductDetailsRatings(product_id, headers):
    try:
        url = details_ratings['name'] + "/" + ratings['endpoint'] + "/" + str(product_id)
        res = requests.get(url, headers=headers, timeout=3.0)
    except BaseException:
        res = None
    if res and res.status_code == 200:
